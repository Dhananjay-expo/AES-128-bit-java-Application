...
/**
 *
 * @author Dhananjay Kuamr 
 * MNNIT ALLAHABAD
 * 
 */
public class Aes_128_bit_algorithm {
     /**
    ...
    private static final int Rcon[] ={0x08, 0x01, 0x02, 0x04, 0x08, 0x10, 0x20, 0x40, 0x80, 0x1b, 0x36};
    ...
    public static byte[] encryptBlock(byte[] in) {
        ...
        for (int i = 0; i < in.length; i++) state[i / 4][i % 4] = in[i%4*4+i/4];
    ...
    public byte[] encrypt(byte[] mssg,byte[] key){

        int lenght=0;
        byte[] padding = new byte[1];
        int i;
        lenght = 16 - mssg.length % 16;    
        padding = new byte[lenght];     
        padding[0] = (byte) 0x80;
  
        for (i = 1; i < lenght; i++) padding[i] = 0;

        byte[] tmp = new byte[mssg.length + lenght];  
        ...
        for (i = 0; i < mssg.length + lenght; i++) {
            if (i > 0 && i % 16 == 0) {
                bloc = encryptBlock(bloc);
                System.arraycopy(bloc, 0, tmp, i - 16, bloc.length);
            }
            if (i < mssg.length)  bloc[i % 16] = mssg[i];
            else{              
